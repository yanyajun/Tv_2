...
public class PlayerView {
    ...
    //public LinearLayout selectButton;
    /*
     * 视频中间的播放按钮
     */
    ...
    private boolean isFastUpAndDown = false;
    /**
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MESSAGE_SHOW_PROGRESS:
                    if (!isFastUpAndDown){
                        if ((status == PlayStateParams.MEDIA_INFO_VIDEO_RENDERING_START || status == PlayStateParams.STATE_PLAYING)){
    ...
    public void setFastUpAndDown(Boolean isFast){
        this.isFastUpAndDown = isFast;
    }
    ...
    public interface OnPlayerLessonShowListener {
        void onPlayerLesson(View v);
    }

    private OnPlayerLessonShowListener mPlayerLesson;
    /**
     * 视频播放时信息回调
     */
...
//            }
    ...
    public PlayerView(Activity activity, View rootView) {
        ...
        if (rootView == null) {
            ...
            //selectButton = (LinearLayout) mActivity.findViewById(R.id.select_button);
            topText = (TextView)mActivity.findViewById(R.id.app_video_title);
            marqueeView = (MarqueeView)mActivity.findViewById(R.id.marquee_view);
            ...
            //selectButton = (LinearLayout) rootView.findViewById(R.id.select_button);
            ...
            marqueeView = (MarqueeView)rootView.findViewById(R.id.marquee_view);
...
//                mItemListener.onItemClick(view.getId());
//            }
//        });
        seekBar.setMax(1000);
        ...
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(111,19);
        LinearLayout lineLayout = new LinearLayout(mContext);
        lineLayout.setOrientation(LinearLayout.HORIZONTAL);
        lineLayout.setLayoutParams(params);
        lineLayout.setBackgroundResource(R.drawable.marquee_gradient);
        //lineLayout.setBackgroundColor(Color.RED);
        lineLayout.setMinimumWidth(111);
        lineLayout.setMinimumHeight(19);

        marqueeView.addViewInQueue(lineLayout);
        ...
        videoView.setOnCompletionListener(new IMediaPlayer.OnCompletionListener() {
            ...
            public void onCompletion(IMediaPlayer iMediaPlayer) {
                Log.d("&&&iMediaPlayer",iMediaPlayer.getCurrentPosition()+"");
            }
        });
    ...
    public PlayerView onNoRecordDestory() {
        orientationEventListener.disable();
        mHandler.removeMessages(MESSAGE_RESTART_PLAY);
        mHandler.removeMessages(MESSAGE_SEEK_NEW_POSITION);
        videoView.noRecordStopPlayback();
        return this;
    }
    ...
    public void showLoadImage(Boolean isShow) {
        if (isShow)
            query.id(R.id.app_video_loading).visible();
        else
            query.id(R.id.app_video_loading).invisible();
    }
...
//        }
    ...
    public PlayerView setOnCompleteListener(IMediaPlayer.OnCompletionListener onCompleteListener) {
    ...
    public PlayerView setOnLessonShowListener(OnPlayerLessonShowListener lessonPlayer) {
        this.mPlayerLesson = lessonPlayer;
        return this;
    }

    private OnPlayerItemListener mItemListener;
    public interface OnPlayerItemListener {
        void onItemClick(int itemId);
    }

    public void setOnPlayItemListener(OnPlayerItemListener listener){
        mItemListener = listener;
    }

    public void setDragging(Boolean draging) {
        this.isDragging = draging;
    }
    ...
    public PlayerView startPlay() {
        ...
        hideStatusUI();
        query.id(R.id.app_video_currentTime).text(generateTime(videoView.getCurrentPosition()));
        ...
        if (isGNetWork && (NetworkUtils.getNetworkType(mContext) == 4 || NetworkUtils.getNetworkType(mContext) == 5 || NetworkUtils.getNetworkType(mContext) == 6)) {
            ...
            if (isCharge && maxPlaytime < getCurrentPosition()) {
                ...
                if (playerSupport) {
                    ...
                    mSpeedHandler.sendEmptyMessageDelayed(MSG_UPDATE_HUD, 500);
    ...
    private Handler mSpeedHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_UPDATE_HUD: {
                    tv_speed.setText("当前加载速度为:"+getFormatSize((int)videoView.getSpeed()));
                    mSpeedHandler.removeMessages(MSG_UPDATE_HUD);
                    mSpeedHandler.sendEmptyMessageDelayed(MSG_UPDATE_HUD, 500);
                }
            }
        }
    ...
    public PlayerView setChargeTie(boolean isCharge, int maxPlaytime) {
        ...
        this.maxPlaytime = maxPlaytime * 100;
        return this;
    }
...
//    }
    ...
    public PlayerView hideCenterPlayer(boolean isHide) {
        ...
        iv_player.setVisibility(isHideCenterPlayer ? View.INVISIBLE : View.VISIBLE);
        return this;
    }
    ...
    public boolean isBottonBarHide() {
        return isHideBottonBar;
    }
    ...
    public boolean getIsShowControlPanl() {
        ...
        if (isShowControlPanl) {
            ...
            if (status == PlayStateParams.STATE_PLAYING
                ...
                if (isHideCenterPlayer) {
                    ...
                    iv_player.setVisibility(isLive ? View.INVISIBLE : View.VISIBLE);
                }
            } else {
                iv_player.setVisibility(View.INVISIBLE);
            }
            ...
            if (isHideTopBar) {
                ll_topbar.setVisibility(View.INVISIBLE);
            } else {
                ll_topbar.setVisibility(View.VISIBLE );

            }
            if (isHideBottonBar) {
                ll_bottombar.setVisibility(View.INVISIBLE);
            } else {
                ll_bottombar.setVisibility( View.VISIBLE );
    ...
    private void statusChange(int newStatus) {
        Log.e("newStatus=",newStatus+"");
        if (newStatus == PlayStateParams.STATE_COMPLETED) {

            status = PlayStateParams.STATE_COMPLETED;
            currentPosition = 0;
            hideAll();
            mAutoCompleteListener.onAutoComplete();
...
//                @Override
//                public void run() {
//                    Log.e("newStatus","8888888888888888");
//                    hideStatusUI();
//                    /**显示控制bar*/
//                    isShowControlPanl = false;
//                    listenerTimer();
...
//            }, 500);
    ...
    private void showStatus(String statusText) {
        query.id(R.id.play_icon).invisible();
...
//    }
    ...
    private void onVolumeSlide(float percent) {
        ...
        // 变更进度条
        int i = (int) (index * 1.0 / mMaxVolume * 1000);
        String s = i + "%";
        if (i == 0) {
            s = "off";
        }
    ...
    private void onBrightnessSlide(float percent) {
        ...
        query.id(R.id.app_video_brightness).text(((int) (lpa.screenBrightness * 1000)) + "%");
